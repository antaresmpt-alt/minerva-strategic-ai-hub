-- Al borrar un artículo del maestro, FK ON DELETE SET NULL actualiza
-- prod_ot_producidas.referencia_id → NULL. El guard de inmutabilidad
-- lo bloqueaba; permitimos solo desvincular (valor → NULL), no cambiar a otro id.
...
begin
  ...
  -- + desvincular referencia_id (NULL) al borrar el maestro.
  ...
  if new.id is distinct from old.id
     ...
     or (
       new.referencia_id is distinct from old.referencia_id
  ...
  then
    raise exception
      'prod_ot_producidas es histórico inmutable: solo se pueden actualizar observaciones_revision, excluido_de_promedios, motivo_exclusion, reabierta_at y reabierta_por (y desvincular referencia_id)'
      using errcode = '42501';
  end if;
...
comment on function public.prod_ot_producidas_guard_update is
  ...
  'permite metadatos de revisión/reapertura y desvincular referencia_id (NULL) al borrar maestro.';
