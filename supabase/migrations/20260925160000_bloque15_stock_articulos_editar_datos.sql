...
-- Revisado Claude 25 sep 2026: OK aplicar (+ notas/condición en audit, clear uds/bulto).
...
create or replace function public.prod_stock_articulos_editar_datos(
  ...
  p_clear_unidades_por_bulto boolean default false,
  p_clear_bultos boolean default false,
  p_clear_pico boolean default false,
  p_clear_palets boolean default false
)
returns void
language plpgsql
security definer
...
begin
  ...
  v_new_bultos := case
    when p_clear_bultos then null
    when p_bultos is not null then p_bultos
    else v_row.bultos
  end;
  v_new_uds_bulto := case
    when p_clear_unidades_por_bulto then null
    when p_unidades_por_bulto is not null then p_unidades_por_bulto
    else v_row.unidades_por_bulto
  end;
  v_new_pico := case
    when p_clear_pico then null
    when p_pico is not null then p_pico
    else v_row.pico
  end;
  v_new_palets := case
    when p_clear_palets then null
    when p_palets is not null then p_palets
    else v_row.palets
  end;
  ...
  if v_new_notas is distinct from v_row.notas then
    v_parts := array_append(
      ...
      format(
        'notas «%s» → «%s»',
        left(coalesce(v_row.notas, '∅'), 60),
        left(coalesce(v_new_notas, '∅'), 60)
      )
    );
  end if;
  if v_new_condicion is distinct from v_row.condicion then
    v_parts := array_append(
      ...
      format(
        'condición «%s» → «%s»',
        left(coalesce(v_row.condicion, '∅'), 60),
        left(coalesce(v_new_condicion, '∅'), 60)
      )
    );
  end if;
