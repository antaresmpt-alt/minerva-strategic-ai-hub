-- Bloque 9.8.4 — Asignar un palet libre (disponible) a una OT STOP.
-- Juan (almacén) ejecuta esto después de que oficina/gerencia haya decidido
-- usar stock existente en lugar de generar una compra nueva (9.8.3).
-- No requiere rol privilegiado — permisos normales de cartelas.
...
comment on function public.prod_stock_asignar_palet_ot(uuid, text, integer, text) is
  'Bloque 9.8.4 — Asigna un palet disponible a una OT STOP: crea bridge row '
  'en prod_stock_palet_ots, registra ajuste en ledger, actualiza estado legacy '
  'y saca la OT del estado STOP (estado_material = ''Material en stock asignado''). '
