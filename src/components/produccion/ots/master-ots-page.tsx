...
export function MasterOtsPage() {
  ...
  const [despachoWizardInitialOt, setDespachoWizardInitialOt] = useState<
    string | null
  >(null);
  const [despachoWizardBatchDefault, setDespachoWizardBatchDefault] =
    useState(false);
  ...
  const openDespachoWizard = useCallback(() => {
    const ot = despachoSeleccion?.num_pedido?.trim() ?? "";
    setDespachoWizardInitialOt(ot || null);
    setDespachoWizardBatchDefault(!ot);
  ...
  return (
    <div className="mx-auto flex max-w-[1600px] flex-col gap-4">
      ...
      <DespachoWizardDialog
        ...
        batchModeDefault={despachoWizardBatchDefault}
        onDespachado={({ ot, rowId }) => {
          setRows((prev) =>
            prev.map((r) =>
              r.id === rowId || String(r.num_pedido ?? "").trim() === ot
                ? { ...r, despachado: true }
                : r
            )
          );
          setDespachoRegistradoOtSet((prev) => {
      ...
      <div className="flex flex-col gap-3 border-b border-slate-200/80 pb-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button
            ...
            size="sm"
            className="gap-1.5"
            onClick={openDespachoWizard}
            title={
              despachoSeleccion
                ? `Despachar OT ${despachoSeleccion.num_pedido}`
                : "Despachar OT (escribe el número en el wizard)"
            }
          >
