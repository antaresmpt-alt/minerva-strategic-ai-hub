...
export function MuelleRecepcionPage() {
  ...
  const [active, setActive] = useState<MuelleCardRow | null>(null);
  ...
  const loadRows = useCallback(async () => {
    ...
    try {
      const { data: compras, error: cErr } = await supabase
        .from(TABLE_COMPRA)
        .select(
          "id, ot_numero, num_compra, material, num_hojas_brutas, proveedor_id, estado"
        )
  ...
  const openSheet = (row: MuelleCardRow) => {
    clearForm();
    setActive(row);
    setSheetOpen(true);
  };
  ...
  const hojasEsperadas = active?.num_hojas_brutas ?? null;
  ...
  const puedeRecepcionTotal = useMemo(() => {
  ...
  const guardarRecepcion = async (modo: "total" | "parcial") => {
    ...
    if (modo === "total" && !puedeRecepcionTotal) {
      toast.error(
        "No se puede cerrar recepción total: las hojas recibidas son menores que las esperadas."
      );
      return;
    }
    if (modo === "parcial" && !notas.trim()) {
      toast.error("Las notas son obligatorias en recepción parcial.");
      return;
    }
    ...
    try {
      ...
      toast.success(
        modo === "total"
          ? "Recepción total registrada y compra cerrada."
    ...
    <div className="mx-auto w-full max-w-6xl space-y-4 px-3 py-4 sm:px-4 md:py-6">
      <header className="space-y-1">
        ...
        <p className="max-w-2xl text-sm text-muted-foreground">
          Recepción de material: compras confirmadas o en curso. Pulsa una tarjeta
          para registrar albarán, bultos y fotos.
        </p>
      </header>
              ...
              <Card className="h-full min-h-[11rem] border-slate-200/90 bg-white shadow-sm">
                ...
                <CardContent className="space-y-2 text-sm">
                  <div className="flex items-baseline justify-between gap-2 border-t border-slate-100 pt-3">
                    <span className="text-muted-foreground">Hojas esperadas</span>
                    <span className="text-lg font-semibold tabular-nums text-[#002147]">
                      {row.num_hojas_brutas != null ? row.num_hojas_brutas : "—"}
                    </span>
                  </div>
        ...
        >
          <SheetHeader className="border-b border-slate-100 pb-3 text-left">
            ...
            <SheetDescription className="text-left text-xs sm:text-sm">
              {active?.material?.trim() || "—"} · Esperadas:{" "}
              {active?.num_hojas_brutas != null ? active.num_hojas_brutas : "—"}{" "}
              hojas
            </SheetDescription>
          </SheetHeader>
          ...
          <div className="flex flex-col gap-4 px-1 py-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="muelle-notas">Notas</Label>
              <Textarea
                id="muelle-notas"
                value={notas}
                ...
                placeholder={
                  puedeRecepcionTotal
                    ? "Opcional si la recepción es total"
                    : "Obligatorio: recepción parcial"
                }
                rows={3}
              ...
              hojasRecNum < hojasEsperadas ? (
                <p className="text-xs text-amber-800">
                  Hojas recibidas por debajo de las esperadas: solo recepción parcial;
                  completa las notas.
                </p>
              ) : null}
            </div>
