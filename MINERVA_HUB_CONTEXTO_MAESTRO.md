...
18. **Calendario Producción (17–18 jul + multi-ámbito 24 jul)**: planificador manual OTs. **Ámbitos** Impresión/Digital/Troquelado/Engomado (`ambito` + letra I/D/T/E); escritura solo admin/gerencia todos, `produccion`→Impresión, sección→la suya; RLS por ámbito. Visibilidad con 4 checks (persistidos). Semáforo pastilla = estado del paso del ámbito en itinerario (`esperando`/`listo`/`hecho`/`sin_paso`); **no** auto-mueve fechas. Tabla `prod_calendario_produccion_ot`. Detalle §15.11–15.13 Bloque 9.
