...
> Complementa `.MANUALES/CONTEXTO/MINERVA_HUB_CONTEXTO_MAESTRO.md`. No describe un bloque funcional concreto, sino una **capa transversal** que afecta a toda la app.
>
> **Estado:** 📋 Diseño / análisis — implementación = **Bloque 12** (`.MANUALES/BLOQUES/MINERVA_BLOQUE12_ROLES_PERMISOS_NAVEGACION.md`). El sistema actual de permisos sigue vigente hasta entonces.
...
**Relacionado:** brief implementación → `.MANUALES/BLOQUES/MINERVA_BLOQUE12_ROLES_PERMISOS_NAVEGACION.md` · permisos de almacén → `.MANUALES/BLOQUES/MINERVA_BLOQUE9_MATERIAL_CARTELAS.md` · roles de planta → `.MANUALES/CONTEXTO/MINERVA_HUB_CONTEXTO_MAESTRO.md`.
...
Emma / Ramón    → Cartelas (crear/imprimir) + Stock + correcciones
Ramón           → autoriza traspasos de reserva
Admin/Gerencia  → todo
```

> **24 jun 2026:** Juan **no** cartela (cuestionario Ramón). Ver `.MANUALES/BRIEFS/MINERVA_CUESTIONARIO_CARTELAS_RAMON.md`.

- Emma y Ramón crean cartelas (`id_stock` único + `created_by`). Antiduplicado por albarán.
...
- **Juan (almacén):** Muelle + Stock (consulta). **No** Cartelas (24 jun). Mobile-first en muelle.
...
- **14 ago 2026:** Ramón confirma **una tableta por máquina** (impresión, troquel, engomado). No personal ni por departamento. Turnos distintos en la misma tableta. Lista gorda de ejecución en `main`; el menú gordo que se ve en tableta es porque se entra de **admin**. Ver `.MANUALES/SESIONES/SESION_14AGO2026_EJECUCION_LISTA.md`.
